...
class Meting
{
    ...
    private function tencent_url($result)
    {
        ...
        $body = [
            ...
            "comm" => [
                'uin'=>(string) $uin, "format"=> "json", "ct"=> 24, "cv"=> 0
            ]
        ];
        $api = array(
            'method' => 'GET',
        ...
        if($purl){
            if(strstr($purl,'http')){
                $url['url'] = str_replace('http://aqqmusic.tc.qq.com/amobile.music.tc.qq.com', 'https://isure.yaode.xyz', $purl);
                // $url['url'] = 'https://isure.stream.qqmusic.qq.com/'.$purl;
            }else{
                // $url['url'] = 'https://isure.stream.qqmusic.qq.com/'.$purl;
                $url['url'] = 'https://isure.yaode.xyz/'.$purl;
            }
        }else{
            $url['url'] = null;
        }
