...
class Index extends Controller
{
    ...
    public function musicUrl($type,$songId){
        ...
        // 读取缓存
        $url = Cache::connect(['path'  =>  '../runtime/musicUrl'])->get('music'.$type.$songId);
        if(!$url){
            $songs = model('admin/Song')->findMusicInfo($type,$songId);
            if ($songs != '' && count($songs) > 0) {
                $song = $songs[0];
                // 设置缓存
                Cache::connect(['path'  =>  '../runtime/musicUrl'])->set('music'.$type.$songId,$song['url'],60*20);
                $url = $song['url'];
            } else {
                $this->error();
            }
    ...
    public function mainColor($url){
        $cache = Cache::connect(['path'  =>  '../runtime/musicColor'])->get('mainColor'.$url);
        if($cache){
            return response($cache);
        }
        ...
        // 设置缓存
        Cache::connect(['path'  =>  '../runtime/musicColor'])->set('mainColor'.$url,$result);
        return response($result);
    }
    ...
    public function musicLyric($type,$songId){
        ...
        // 读取缓存
        $cache = Cache::connect(['path'  =>  '../runtime/musicLyric'])->get('musicLyric'.$type.$songId);
        $script = "var lrcstr =''";
        if(!$cache){
            $songs = model('admin/Song')->findMusicInfo($type,$songId);
            if ($songs != '' && count($songs) > 0) {
            ...
            // 设置缓存
            Cache::connect(['path'  =>  '../runtime/musicLyric'])->set('musicLyric'.$type.$songId,$script);
        }
